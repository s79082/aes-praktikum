#include "../src/aes.h"
#include <stdio.h>
#include <string.h>

int main() {
  u_int8_t input[16] = {0x00, 0x00, 0x01, 0x01, 0x03, 0x03, 0x07, 0x07,
                        0x0f, 0x0f, 0x1f, 0x1f, 0x3f, 0x3f, 0x7f, 0x7f};

  u_int8_t block[16];
  memcpy(block, input, 16);

  u_int8_t roundkeys[176] = {
      };

  // expected c7d12419 489e3b62 33a2c5a7 f4563172
  u_int8_t desired_result[] = {0xc7, 0xd1, 0x24, 0x19, 0x48, 0x9e, 0x3b, 0x62,
                               0x33, 0xa2, 0xc5, 0xa7, 0xf4, 0x56, 0x31, 0x72};

  encrypt(block, roundkeys, 10);

  for (int i = 0; i < 16; i++) {
    if (block[i] != desired_result[i]) {
      printf("Error at index %d\n", i);
      printf("Expected: %x\n", desired_result[i]);
      printf("Got: %x\n", block[i]);
      return 1;
    }
  }
  return 0;
}